...
let test_deserialize_state () =
  let json_string = In_channel.with_open_text "example.tfstate" In_channel.input_all in
...
let test_deserialize_complex_state () =
  let json_string = In_channel.with_open_text "complex.tfstate" In_channel.input_all in
  let yojson = Yojson.Safe.from_string json_string in
  let state = Terraform_state.state_v4_of_yojson yojson in
  ...
  run
    ...
    [ ( "deserialization"
      , [ test_case "Deserialize example state" `Quick test_deserialize_state
        ; test_case "Deserialize complex state" `Quick test_deserialize_complex_state
        ] )
    ]